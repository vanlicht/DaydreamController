        ...
        else
        {
          // Change material to reflect button presses.
            ...
            if (GvrController.IsTouching)
            {
                ...
                Vector3 scale = Vector3.Lerp(Vector3.zero, TOUCHPAD_POINT_DIMENSIONS, elapsedScaleTimeSeconds / TOUCHPAD_POINT_SCALE_DURATION_SECONDS);
                ...
                //T: for swiping tiltbrush ui
                Vector2 touchPos = GvrController.TouchPos;

                float dist = Vector2.Distance(touchPoint.transform.localPosition, oldTouchPoint) * 100f;
                //Debug.Log("previousTouchValue: " + previousTouchValue.x);
                //Debug.Log("curTouchValue: " + curTouchValue.x);
                //Debug.Log("distance: " + dist);
                if (dist > 0.05f && !isAnimating)
                {
                    isAnimating = true;
                    float rotAmount;
                    

                    if (oldTouchPoint.x > touchPoint.transform.localPosition.x)
                    {
                        rotAmount = -90f;
                    }
                    else
                    ...
                    Debug.Log(rotAmount);
                    //To do: rotate the UI
                    //To do: set isAnimating to false once rotation is done
                }

                oldTouchPoint = touchPoint.transform.localPosition;
            ...
            else
            {
                ...
                Vector3 scale = Vector3.Lerp(TOUCHPAD_POINT_DIMENSIONS, Vector3.zero, elapsedScaleTimeSeconds / TOUCHPAD_POINT_SCALE_DURATION_SECONDS);

                touchPoint.transform.localScale = scale;

                //T: reset the previous Touch value
                oldTouchPoint = touchPoint.transform.localPosition;
                isAnimating = false;
            }
